...
<form action="{{ route('contacts_update', ['id' => $posts->id]) }}" method="POST">
  ...
  <div class="contactsedit">
    ...
    <div class="contactsItem"> <select name="status" class="Item-Input-ex">
        <option value="Outstanding" {{ old('status',
      $posts->status) === 'Outstanding' ? 'selected' : ''}}>未対応</option>
        <option value="Processing" {{ old('status', $posts->status) === 'employee' ? 'Processing' : ''}}>対応中</option>
        <option value="Closed" {{ old('status', $posts->status) === 'self-employed' ? 'Closed' : ''}}>対応済</option>
      </select>
    </div>

    <p><b>お問い合わせ内容</b></p>
    <div class="contactsItem"> {!! nl2br(e($posts['contact_body'])) !!} </div>
